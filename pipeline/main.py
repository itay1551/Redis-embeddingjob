...
# S3 bucket
@dsl.component(base_image='registry.access.redhat.com/ubi9/python-311', packages_to_install=['boto3', 'langchain-community', 'pypdf'])
    ...
    while True:
        try:
            ...
            if continuation_token:
                response: dict= s3.list_objects_v2(Bucket=bucket_name, Prefix=folder_path + '/',ContinuationToken=continuation_token)
            else:
                response: dict= s3.list_objects_v2(Bucket=bucket_name, Prefix=folder_path + '/')
...
def split_and_embed(input_docs_path: dsl.InputPath()):
    ...
    # return dsl.ContainerSpec(
        ...
        # command=['pip', 'install', 'langchain-community', 'langchain'], 
        # args=[]
        # args=['langchain-community', 'langchain'])
...
def rag_llm_pipeline(is_load_from_repo: bool, is_load_from_s3: bool, is_load_from_urls: bool, TODO_must_update_secret_ds_pipeline_config_llm_rag: str =''):
